...
arch -x86_64 "$WINE_APP" cmd /c \
  "M: && cd \\MWOS\\TMP && set MWOS=M:\\MWOS && set PATH=M:\\MWOS\\DOS\\BIN;%PATH% && M:\\MWOS\\DOS\\BIN\\xcc.exe -mw=M:\\MWOS -tp=68030,ld -olM=${STACK_KB}K -f=${STAGE_WIN}\\${MODULE} ${STAGE_WIN}\\qcc_p.c"
