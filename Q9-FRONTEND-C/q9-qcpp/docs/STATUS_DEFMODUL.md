...
| **2. Minimaler IR-Metadatenpfad** | Metadaten sicher bis zum 68k-Backend | 🟡 | Emission (mit Defaults) + QCCVM + 68k-Backend-Erkennung erledigt; dabei Architekturfund: echter Modulkopf kommt von `q9_cstart.a`, nicht von der `psect`-Zeile. IR-Validierung, ARM64/C-Backend-Ablehnung noch offen |
...
| ARM64-/C-Backend: klare „noch nicht implementiert"-Ablehnung | 🔴 | Noch nicht begonnen — Build-Pfad für `Q9-BACKEND-ARM64/q9-qirarm64` (kein Makefile im erwarteten Verzeichnis gefunden) und der genaue Ort des "C-Backends" aus 1.d/2.4 müssen erst geklärt werden, bevor dort gezielt abgelehnt werden kann. Niedrigere Priorität als der 68k-Fund oben, da hier nichts crasht (nur potenziell falsches Verhalten bei tatsächlicher Nutzung — noch niemand nutzt `#DEFMODUL` mit diesen Zielen). |
