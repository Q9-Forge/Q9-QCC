...
`xcc`. See "Selfhosting loop closed" further down.
...
fundamentally unable to run on the Q9 (16 MB):
...
Three rebuilds, each sized against **real data** rather than guessed:
`Instr` 416 -> 80 bytes (fixed argument text fields -> pointers into a
pool), `Global` 16 KB -> 40 bytes (initialiser buffer -> pool, allocated
lazily on the first `GINIT`), and `actionLog` 1048576 -> 262144 entries
(measured peak demand on the real target: 154306). As separate processes
both now fit.

Found and fixed along the way -- a pre-existing **wrong-code bug**: a global
array longer than `MAX_ARRAY_LEN` **with** an initialiser emitted memory
from beyond the buffer as assembler data (reproducibly `1751343470`, which
is `"nach"`, the start of the neighbouring name). Silent, no crash.
Regression test `M4a-oob` with a counter-probe.

### Two errors that made every measurement unreliable

**Semantic errors were not fatal.** See the detailed account in
`SELFHOSTING_GAP_LIST.md`. In short: the generated parser reported errors on
stderr but exited 0 and printed `OK`. A generated parser now closes with one
of three words -- `OK` / `SEMERR` / `FAIL`, exit `0` / `1` / `1` -- and the
distinction is load-bearing, see README.

**A function-pointer call through a plain variable emitted no call at all.**
`Fn f; f = g; f(7);` produced `PUSHFN g / STOREP 0 / PUSH 7 / DROP`. It
survived because `tools/qccvm.py`, the IR reference oracle behind
`tc_check`, knew neither `PUSHFN` nor `CALLIND` -- function pointers were
never covered by the suite.
...
the backend blocker "IR line 4265: negative local slot".
