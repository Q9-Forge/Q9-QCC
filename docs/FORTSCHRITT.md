...
2,1-MB-OS-9-Modul. **Der komplette QCC-Vollport von `Source/ebnf.cpp` ist
...
| QCC `switch`/`case`/`default` | teilweise | gestapelte Case-Label (case A: case B: body) unterstuetzt, KEIN Fallthrough mit Code zwischen verschiedenen Bodies (deckt das reale Nutzungsmuster in ebnf.cpp/codegen.cpp ab); kein Backend-Change |
