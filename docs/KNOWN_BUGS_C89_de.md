...
| Funktionszeiger als lokale Variable | **BEHOBEN 2026-09-15.** `int (*fp)(int);` ohne den Umweg über ein `typedef`, auch mit `void`-Rückgabe und mehreren Parametern. Vorher stiller Parse-Abbruch. **Weiterhin offen** sind die drei anderen Stellen — als Parameter, als globale Variable und als Struct-Feld; dort führt der `typedef`-Weg (`typedef int (*FP)(int);`) seit jeher zum Ziel und funktioniert unverändert. | teilweise |
