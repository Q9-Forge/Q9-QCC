...
echten Q9 und erzeugt eine mit der `xcc`-gebauten Referenz BITGLEICHE
Parserausgabe. Siehe "Selfhosting-Kreis geschlossen" weiter unten.
...
waren auf dem Q9 (16 MB) grundsätzlich nicht lauffähig:

| | vorher | nachher | auf 68k |
|---|---:|---:|---:|
| `Source/qcc_backend_c.cpp` | 42,2 MB | **6,4 MB** | ~4,7 MB |
| `Data/qcc_p.c` | 25,7 MB | **6,5 MB** | ~3,3 MB |

Drei Umbauten, jeder an **echten Daten** bemessen statt geschätzt:
`Instr` 416 -> 80 Byte (feste Argument-Textfelder -> Zeiger in einen Pool),
`Global` 16 KB -> 40 Byte (Initialisierer-Puffer -> Pool, erst beim ersten
`GINIT` belegt) und `actionLog` 1048576 -> 262144 Einträge (gemessener
Spitzenbedarf am echten Ziel: 154306). Als getrennte Prozesse passen beide
jetzt hinein.

Dabei gefunden und behoben -- ein vorbestehender **Falschcode-Fehler**: ein
globales Array länger als `MAX_ARRAY_LEN` **mit** Initialisierer gab
Speicher hinter dem Puffer als Assemblerdaten aus (reproduzierbar
`1751343470`, das ist `"nach"`, der Anfang des Nachbarnamens). Still, ohne
Absturz. Regressionstest `M4a-oob` mit Gegenprobe.

### Zwei Fehler, die jede Messung unbrauchbar machten

**Semantische Fehler waren nicht tödlich.** Ausführlich in
`SELFHOSTING_GAP_LIST_de.md`. Kurz: der erzeugte Parser meldete Fehler auf
stderr, endete aber mit 0 und schrieb `OK`. Ein erzeugter Parser schließt
jetzt mit einem von drei Worten -- `OK` / `SEMERR` / `FAIL`, Rückgabewert
`0` / `1` / `1` -- und die Unterscheidung trägt, siehe README.

**Ein Funktionszeiger-Aufruf über eine einfache Variable erzeugte gar keinen
Aufruf.** `Fn f; f = g; f(7);` ergab `PUSHFN g / STOREP 0 / PUSH 7 / DROP`.
Überlebt hat das, weil `tools/qccvm.py`, das IR-Referenzorakel hinter
`tc_check`, weder `PUSHFN` noch `CALLIND` kannte -- Funktionszeiger waren
nie durch die Suite abgedeckt.

### Gemessener Stand

**Stufe 1 (Grammatik) am Bootstrap-Ziel ist erstmals leer: 768 von 768
Einheiten.** Dorthin führten zwei geschlossene Lücken — der Komma-Operator
(2026-08-11) und der Memberzugriff auf Funktionsrückgabewerte
(`callMember`/`tc_callmember`, parallel in einer anderen Sitzung
entstanden). Letzterer schloss vier Einheiten auf einmal, weil alle vier am
selben Konstrukt scheiterten.

**Stufe 2 (Semantik) ist damit erstmals messbar**: 30 verschiedene
Beanstandungen. Mehrere davon sind Artefakte des Messaufbaus (die
Vorverarbeitung streicht `#include`, daher Phantom-`unknown function` und
`unknown type name 'FILE'`), die Liste ist also mit Vorsicht zu lesen —
Einzelheiten und die echten Lücken in `SELFHOSTING_GAP_LIST_de.md`.

Eine der Beanstandungen ist selbstverschuldet und kommt zuerst: die erklärte
Grenze des Komma-Operators („das letzte Glied muss einen Wert liefern")
bricht gewöhnliches C — `if ((p = f()))` und `while ((c = g()))`. Eine
geklammerte Zuweisung muss den zugewiesenen Wert liefern.

Darüber hinaus offen: `inputFileBuf` fasst 256 KB, `Data/qcc_p.c` ist ~300 KB
groß (vorverarbeitet passt es, wächst aber mit jeder Grammatikänderung),
sowie der Backend-Blocker "IR Zeile 4265: negativer lokaler Slot".
