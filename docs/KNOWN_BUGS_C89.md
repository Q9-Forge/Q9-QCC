...
| Function pointer as a local variable | **FIXED 2026-09-15.** `int (*fp)(int);` without the detour through a `typedef`, including `void` return and several parameters. Previously a silent parse failure. **Still open** are the three other positions - as a parameter, as a global, and as a struct field; there the `typedef` route (`typedef int (*FP)(int);`) has always worked and is unchanged. | partial |
