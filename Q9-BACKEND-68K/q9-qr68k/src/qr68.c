...
/* add/sub/and/or/eor/cmp in all forms generated by r68. */
static void doArith(const char *base, int size)
{
	...
	/* Address register as destination -> A-form. */
	if (oMode[1] == AM_AN) {
		if (!isAdd && !isSub && !isCmp)
	...
	/* Immediate source -> Q-form (add/sub only, values 1..8) or I-form. */
	if (oMode[0] == AM_IMM) {
		needAlterable(1);
...
static void doShift(int type, int dr, int size)
{
	...
	if (oN < 1 || oN > 2)
		fatal("falsche Zahl von Operanden: ", lnOp);
	if (oN == 1) {
		/* Memory form: exactly one bit, always word size. */
		if (size != 0 && size != 'w')
...
/* One sized operand: clr/neg/negx/not/tst. */
