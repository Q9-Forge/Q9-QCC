#!/usr/bin/env bash
# Run a public-interface regression against host libc and the 68030.
# Usage: bash tests/libc68k.sh tests/memcmp.c
set -euo pipefail
SOURCE=${1:?test source required}
...
WORK=$(mktemp -d /tmp/qclib-libc.XXXXXX)
mkdir -p "$WORK/host"
...
cp "$SOURCE" "$WORK/host/fgtest.c"
( cd "$WORK/host" && cc -w -O1 -fno-builtin -o fgh fgtest.c ) || die "clang"
( cd "$WORK/host" && ./fgh ) > "$WORK/host.out" 2>&1 || die "Hostlauf"
sed 's/^/    /' "$WORK/host.out"

echo "== 2/4 gegen qclib binden (eigene Kette) =="
"$QCC/Q9-FRONTEND-C/q9-qcpp/build/qcpp" -I"$REPO/../../Q9-FRONTEND-C/q9-qcpp/include" "$SOURCE" "$WORK/fg.i" || die "qcpp"
...
# haengengeblieben).
tr -d '\r' < "$WORK/run.log" |
	grep -E '^CASE ' > "$WORK/ziel.out" || true
sed 's/^/    /' "$WORK/ziel.out"
echo
if diff -q "$WORK/host.out" "$WORK/ziel.out" >/dev/null; then
	echo "  *** ZEICHENGLEICH zum Hostlauf -- public interface verified ***"
	exit 0
fi
echo "  ABWEICHUNG:"
