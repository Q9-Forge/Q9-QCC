/*
 * PRUEFKOPIE – NICHT TEIL DES AKTIVEN BACKENDS.
 * Diese unversionierte Arbeitsdatei stammt aus dem lokalen Q9-Parsec-
 * Arbeitsbaum. Sie ist deutlich größer als die aktive Version und muss noch
 * auf wertvollen oder fehlenden Code geprüft werden. Bis dahin nicht bauen
 * und nicht in Bootstrap- oder Testpfade aufnehmen.
 */
//================================================================================
		   ...
		   Im echten Generator kommt dieser Fall nicht vor. */
		{
			...
			for (pslot = 0; pslot < fn->nargs; pslot++) {
				...
				for (pk = fn->first; pk < fn->last; pk++) {
					Instr* px = &ir[pk];
					if (px->argc == 1 && number(px->args[0], px->line) == pslot) {
						if (strcmp(px->op, "LOADC") == 0 || strcmp(px->op, "STOREC") == 0) usedAsChar = 1;
						/* 2026-09-09, short: dieselbe Falle wie bei char, nur das
						   LOW WORD (Byte 2-3 des 4-Byte-Slots) statt das LOW BYTE
						   (Byte 3). */
						else if (strcmp(px->op, "LOADLH") == 0 || strcmp(px->op, "STORELH") == 0) usedAsShort = 1;
					}
				}
 ...
 * PRUEFKOPIE – NICHT TEIL DES AKTIVEN BACKENDS.
 *
 * Diese unversionierte Arbeitsdatei wurde am 2026-09-11 aus dem lokalen
 * Q9-Parsec-Arbeitsbaum kopiert. Sie ist deutlich groesser als die aktive
 * qcc_backend.cpp-Version und muss deshalb noch inhaltlich auf wertvollen
 * oder fehlenden Code untersucht werden. Bis zum Abschluss dieses Vergleichs
 * darf sie weder gebaut noch in Bootstrap-/Testpfaden verwendet werden.
 */
